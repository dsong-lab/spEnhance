...
from utils import load_image, load_tsv, read_lines, read_string, load_csv
...
def main():
    ...
    infile_cnts = f'{prefix}cnts_aug.csv'
    infile_locs = f'{prefix}locs_aug.csv'
    infile_img = f'{prefix}he.jpg'
    infile_genes = f'{prefix}gene-names.txt'
    infile_radius = f'{prefix}radius.txt'

    # load data
    cnts = load_csv(infile_cnts)
    locs = load_csv(infile_locs)
