...
def poisson_loss(predicted_count, true_count, eps=1e-8, reduction='mean'):
    """
    Poisson negative log-likelihood loss.
    
    Args:
        predicted_count: Tensor of shape (N, G), model-predicted counts.
        true_count: Tensor of shape (N, G), ground-truth counts.
    ...
    """
    predicted_count = torch.clamp(predicted_count, min=eps)  # 防止 log(0)
    loss = predicted_count - true_count * torch.log(predicted_count)
...
def cosine_similarity_loss(pred_spot_expr, gt_spot_expr):
    similarity = F.cosine_similarity(pred_spot_expr, gt_spot_expr, dim=-1)  # per-spot similarity
...
class scstGCN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_huber = F.smooth_l1_loss(y_mean_pred, y_mean, beta=1.0).mean()
        loss_cos = cosine_similarity_loss(y_mean_pred, y_mean).mean()
        # poisson_loss = torch.nn.PoissonNLLLoss(log_input=False)
        # loss_poss = poisson_loss(y_mean_pred, y_mean).mean()
        loss = loss_huber + 0.1 * loss_cos
